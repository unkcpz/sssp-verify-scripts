from sssp_verify_scripts.controllers import ConvergencePressureGroupSubmissionController
...
if mode == "experimental":
    ...
    target_group_label=f"experimental/convergence/{property}/{protocol}/{conf.lower()}"
elif mode == "validate":    
    # Validate on the candidate PPs: SSSP-prec-v1.3 recollected
    target_upf_lib = "validate/upf/candidate/sssp-prec-v1.3"
    target_group_label=f"validate/sssp-prec-v1.3/convergence/{property}/{protocol}/{conf.lower()}"
else:
    raise ValueError(f"Unknown mode {mode}")

comment = "{property}-convergence-protocol-compare"
...
print(f"\nLaunching Convergence {property} controller ---\n")
try:
    controller = ConvergencePressureGroupSubmissionController(
