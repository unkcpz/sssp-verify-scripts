...
import sys
import copy
import common.quantities_for_comparison as qc
...
ALL_ELECTRON_CODES = ["AE-average|WIEN2K+FLEUR"]
    ...
    for quantity_name in quantity_names:
        ...
        for code_label in used_code_labels:
            ...
            for set_name in set_names:
                reference_data_files = labels_data['methods-main']['PseudoDojo-v0.5b1|abinit@ACWF']
                ...
                for element_and_configuration in plot_systems:
                    ...
                    if (code_label == 'PseudoDojo-v0.4.1|qe@SSSP' or code_label == 'PseudoDojo-v0.5|qe@SSSP' or code_label == 'PseudoDojo-v0.5b1|abinit@ACWF') and quantity_name == '% difference in V0' and abs(quantity_value) > 0.5:
                        print(f"{quantity_name} outliers should not happened {element}-{configuration} for {code_label}, value={quantity_value}")
                        if element in ['Cu', 'Zn', 'Ne', 'Ar', 'Cd']:
                            continue

                    if (code_label == 'PseudoDojo-v0.4.1|qe@SSSP' or code_label == 'PseudoDojo-v0.5b1|abinit@ACWF') and quantity_name == '% difference in B0' and abs(quantity_value) > 10:
                        print(f"{quantity_name} outliers should not happened {element}-{configuration} for {code_label}, value={quantity_value}")
                        if element in ['Cu', 'Zn', 'Ne']:
                            continue

                    if (code_label == 'PseudoDojo-v0.4.1|qe@SSSP' or code_label == 'PseudoDojo-v0.5b1|abinit@ACWF') and quantity_name == '% difference in B1' and abs(quantity_value) > 40:
                        print(f"{quantity_name} outliers should not happened {element}-{configuration} for {code_label}, value={quantity_value}")
                        if element in ['Cu', 'Zn', 'Ne']:
                            continue
...
if __name__ == "__main__":
    ...
    if mode == 'together':
        generate_box_plt([SET_NAME_1, SET_NAME_2], 'dojo-box_plot_',
        ...
        generate_box_plt([SET_NAME_1], 'dojo-box_plot_unaries_',
            only_must_have_elements=only_must_have_elements, material_set_label=material_set_label, file_suffix=elements, keep_only_codes=keep_only_codes)
        generate_box_plt([SET_NAME_2], 'dojo-box_plot_oxides_',    
            only_must_have_elements=only_must_have_elements, material_set_label=material_set_label, file_suffix=elements, keep_only_codes=keep_only_codes)
    else:
        print("Pass as first parameter 'separate' (separate analysis for unaries and oxides) or 'together'.")
        sys.exit(1)
