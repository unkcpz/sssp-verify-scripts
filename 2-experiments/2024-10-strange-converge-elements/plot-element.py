...
def plot(element, converge_h5, conff='gs'):
    count = 0
    plot_lines = []
    offset = 8
    ...
    plt.xlabel(f'Wavefunction cutoff [Ry]; Charge density cutoff [Ry] = xx x Ewfc (PAW/US) | 4 x Ewfc (NC); q-point = '+str([0.5, 0.5, 0.5]),fontsize=20)
    ...
    Path(f'plots_{CRITERIA}').mkdir(exist_ok=True)
    plt.savefig(f'plots_{CRITERIA}/{element}_{conff}_{CRITERIA}_summary.png')
    plt.close()

            
if __name__ == "__main__":
    # Load the dataset of convergence results
    converge_h5 = h5py.File(f'./pp_verify_convergence.h5')
