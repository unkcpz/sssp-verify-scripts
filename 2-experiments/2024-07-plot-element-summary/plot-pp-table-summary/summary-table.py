...
def summary(element):
    ...
    for pp_name in pps:
        ...
        for conff in ['bcc', 'fcc', 'dc']:
            rec_cutoff = compute_rec_cutoff(conff, pp_name, converge_h5)      

            for cri in ['efficiency', 'precision']:
                ereport[f'{conff}.{cri}'] = rec_cutoff[cri]

            # end of conff loop

        # compute score eff/prec
        nu_wo_max = ereport['avg.nu.w/o.max']
        ele = pp_name.split('.')[0]
        stable_conf = conf_mapping[ele]
        
        # eff
        eff_ffco = ereport[f'{stable_conf}.efficiency']

        # prec
        prec_ffco = ereport[f'{stable_conf}.precision']

        if ereport['nu_prec_score'] is not None and prec_ffco is not None:
            ereport['prec_score'] = ereport['nu_prec_score'] + prec_ffco / 200.0
        else:
            ereport['prec_score'] = None

        if ereport['nu_eff_score'] is not None and eff_ffco is not None:
            ereport['eff_score'] = ereport['nu_eff_score'] + eff_ffco / 100.0
        else:
            ereport['eff_score'] = None
...
if __name__ == "__main__":
    ...
    with open('table-report.json', 'w') as outfile:
