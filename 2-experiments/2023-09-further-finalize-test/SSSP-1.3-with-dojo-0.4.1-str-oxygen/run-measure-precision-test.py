...
comment = "SSSP-1.3-precision-with-dojo-0.4-str-oxygen"
...
dojo_base_path = "/home/jyu/Projects/WP-SSSP/sssp-verify-scripts/libraries-pbe/NC-DOJOv0.4-stringent"
oxygen_pseudo_path = os.path.join(dojo_base_path, "O.nc.z_6.oncvpsp3.dojo.v0.4.1-str.upf")
...
for pseudo, (wfc_cutoff, rho_cutoff) in pseudos.items():
    ...
    command = f"aiida-sssp-workflow launch --property measure.precision --configuration XO --configuration XO2 --configuration XO3 --configuration X2O --configuration X2O3 --configuration X2O5 --oxygen-pseudo {oxygen_pseudo_path} --oxygen-ecutwfc {dojo_oxygen_ecutwfc} --oxygen-ecutrho {dojo_oxygen_ecutrho} --ecutwfc {wfc_cutoff} --ecutrho {rho_cutoff} --pw-code pw-7.0@{computer} --protocol acwf --withmpi True --num-mpiprocs {mpiprocs} --npool {npool} --comment {comment} -- {pseudo_path}"
    os.system(command)
