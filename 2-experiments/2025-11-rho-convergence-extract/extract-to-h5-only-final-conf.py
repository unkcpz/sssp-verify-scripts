...
def extract_convergence(lib_name, property, f_compute_xy, override=False):
    with h5py.File(f"pp_verify_convergence_final_conf.h5", "a") as f:
        group_name = f"validate/{lib_name}/convergence/{property}/standard/the-stable"
        ...
        for node in tqdm(group.nodes, file=sys.stdout):
            ...
            if node.exit_status != 0:
                eprint(
                    f"node {node.uuid} verify on {filename} on/ the stable conf, property {property} not okay"
                )
                continue
